function doEDAPlots(res_table)
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here
%%
figure
subplot(3,1,1)
plot(res_table.Time, res_table.Detection_X, 'rx')
title('Detection distances from odom over time')

xlabel('Time (s)')
ylabel('Detection X (m)')
subplot(3,1,2)
plot(res_table.Time, res_table.Detection_Y, 'rx')
xlabel('Time (s)')
ylabel('Detection Y (m)')
subplot(3,1,3)
plot(res_table.Time, res_table.Detection_Z, 'rx')
xlabel('Time (s)')
