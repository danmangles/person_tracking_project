...
class RealsenseDetector:
    def __init__(self, in_simulator, verbose):
        ...
        self.skip = 0
        self.yolo_net_dimension = 416 # we are using a CNN which takes inputs of size 416x416
    ...
    def rgb_callback(self, data):
        '''
        input: data: an image in sensor_msgs/CompressedImage format
        ...
        frame = self.bridge.compressed_imgmsg_to_cv2(data, "bgr8") #frame is a new image in cv2 format
        if self.verbose:
    ...
    def preprocessing(self, input_image):
        ...
        resized_image = cv2.resize(input_image, (self.yolo_net_dimension, self.yolo_net_dimension), interpolation = cv2.INTER_CUBIC)
